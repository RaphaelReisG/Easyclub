...
const itemOrcamentos = usePage().props.itemOrcamentos;

const form = useForm({
    name: orcamento.name,
    description: orcamento.description,
    //cliente_id: orcamento.clente_id,
    itens: itemOrcamentos,
});
...
const submit = () => {
    if (form.itens.length > 0) {
        var t = orcamento.id
        //form.post(route('orcamento.update'));
        form.put(route('orcamento.update', {id: t}));
    } else {
        ...
        <template #header>
            <div>
                <h2 class="inline-block font-semibold text-xl text-gray-800 leading-tight">
                    <Link class="underline" :href="route('orcamento.index')">Orçamentos</Link> > Editar
                </h2>
            </div>
        </template>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <div v-if="$page.props.auth.user.userable_type === 'App\\Models\\Cliente'" class="p-6 text-gray-900">
                        <p>Preencha o formulario abaixo para editar um orçamento no sistema.</p>
                        ...
                        <div>
                            ...
                            <div class="flex space-x-4">
                                ...
                                <SelectObject rotulo="" lista="tipoOrcamento"
                                    placeholder="Selecione o tipo do orçamento" name="tipo_orcamento_id"
                                    v-model="dataItem.tipo_orcamento_id"></SelectObject>
                                <inputNew rotulo="" placeholder="Nome do Item" name="nameItem" type="text"
                                    v-model="dataItem.name" required></inputNew>
                                <inputNew style="max-width: 20px;" rotulo="" placeholder="Quantidade" name="QuanttityItem" type="number"
                                    v-model="dataItem.quantity" required></inputNew>
